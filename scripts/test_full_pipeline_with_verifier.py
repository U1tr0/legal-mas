...
def main() -> None:
    ...
    ap.add_argument("query", help="User query to test full pipeline with verifier")
    ...
    verification = verifier.verify(args.query, plan, retrieval_result, generated_answer)

    print("=== PLANNER OUTPUT ===")
    print(json.dumps(plan.model_dump(), ensure_ascii=False, indent=2))

    print("\n=== EVIDENCE PACK ===")
    print(json.dumps(retrieval_result["evidence_pack"], ensure_ascii=False, indent=2))
    ...
    if verification.verdict == "ACCEPT":
        print("Final answer can be returned to the user.")
    elif verification.verdict == "REWRITE":
        print("Answer should be rewritten.")
    elif verification.verdict == "RETRIEVE_MORE":
        print("Retrieval evidence is insufficient; need more materials.")
    elif verification.verdict == "ASK_USER":
        print("Need clarification from the user.")
    elif verification.verdict == "REFUSE":
        print("Request is out of scope or should be refused.")
